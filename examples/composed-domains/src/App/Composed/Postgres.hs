...
composedDatabaseChanges =
  [ DatabaseChange
    ...
    DatabaseChange
      { databaseChangeId = DatabaseChangeId "runtime-role-grants-v3",
        databaseChangeStatements =
          NonEmpty.fromList
            [ "GRANT USAGE ON SCHEMA composed TO web_api_runtime;",
              "GRANT SELECT, UPDATE (last_used_totp_counter) ON composed.admission_credentials TO web_api_runtime;",
              "GRANT SELECT, INSERT, UPDATE ON composed.admission_sessions TO web_api_runtime;",
              "GRANT SELECT, INSERT, UPDATE, DELETE ON composed.csrf_synchronizer_tokens TO web_api_runtime;",
