...
spec = do
  ...
  describe "signed CSRF protection" $ do
    it "validates bounded key configuration without rendering signing material" $ do
      let keyId = required "CSRF key ID" (Signed.mkCsrfKeyId "current")
          otherKeyId = required "other CSRF key ID" (Signed.mkCsrfKeyId "previous")
          signingKey = required "CSRF signing key" (Signed.mkCsrfSigningKey "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
      expectAll
        ( (Signed.mkSignedCsrfKeyring keyId ((keyId, signingKey) :| []) `shouldSatisfy` isJust)
            :| [ Signed.mkCsrfKeyId "" `shouldBe` Nothing,
                 Signed.mkCsrfKeyId "key with space" `shouldBe` Nothing,
                 Signed.mkCsrfKeyId (Text.replicate 33 "a") `shouldBe` Nothing,
                 Signed.mkCsrfSigningKey "not-base64url" `shouldBe` Nothing,
                 Signed.mkCsrfSigningKey "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA" `shouldBe` Nothing,
                 Signed.mkSignedCsrfKeyring otherKeyId ((keyId, signingKey) :| []) `shouldSatisfy` isNothing,
                 Signed.mkSignedCsrfKeyring keyId ((keyId, signingKey) :| [(keyId, signingKey)]) `shouldSatisfy` isNothing,
    ...
    it "requires positive signed-token lifetime and clock-skew policy" $
      expectAll
        ( (Signed.mkSignedCsrfPolicy 1 1 `shouldSatisfy` (/= Nothing))
            :| [ Signed.mkSignedCsrfPolicy 0 1 `shouldBe` Nothing,
                 Signed.mkSignedCsrfPolicy 1 0 `shouldBe` Nothing
               ]
        )
    ...
    it "binds a signed token to current application grants without exposing its value" $ do
      ...
      let keyId = required "CSRF key ID" (Signed.mkCsrfKeyId "current")
          signingKey = required "CSRF signing key" (Signed.mkCsrfSigningKey "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
          keyring = required "CSRF key ring" (Signed.mkSignedCsrfKeyring keyId ((keyId, signingKey) :| []))
            ...
            Signed.signedCsrfProtection
              Signed.SignedCsrfDependencies
                { Signed.signedCsrfDependenciesKeyring = keyring,
                  ...
                  Signed.signedCsrfDependenciesCurrentTime = readIORef currentTime,
                  Signed.signedCsrfDependenciesResolveBinding = \isActive -> pure (if isActive then Csrf.BoundCsrfBinding sessionBinding activeExpiry else Csrf.AnonymousCsrfBinding)
                }
            ...
            Signed.signedCsrfProtection
              Signed.SignedCsrfDependencies
                { Signed.signedCsrfDependenciesKeyring = keyring,
                  ...
                  Signed.signedCsrfDependenciesResolveBinding = const (pure Csrf.CsrfBindingUnavailable)
                }
    ...
    it "rejects malformed, unavailable, stale, and structurally invalid signed-token states" $ do
      ...
      let currentKeyId = required "current CSRF key ID" (Signed.mkCsrfKeyId "current")
          otherKeyId = required "other CSRF key ID" (Signed.mkCsrfKeyId "other")
          signingKey = required "CSRF signing key" (Signed.mkCsrfSigningKey "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
          otherSigningKey = required "other CSRF signing key" (Signed.mkCsrfSigningKey "MTExMTExMTExMTExMTExMTExMTExMTExMTExMTExMTE")
          keyring = required "CSRF key ring" (Signed.mkSignedCsrfKeyring currentKeyId ((currentKeyId, signingKey) :| []))
          otherKeyring = required "other CSRF key ring" (Signed.mkSignedCsrfKeyring otherKeyId ((otherKeyId, otherSigningKey) :| []))
          policy = required "CSRF policy" (Signed.mkSignedCsrfPolicy 1000 1)
    ...
    it "handles clock arithmetic bounds without accepting an expired token" $ do
      let keyId = required "CSRF key ID" (Signed.mkCsrfKeyId "current")
          ...
          largeSkewPolicy = required "large skew policy" (Signed.mkSignedCsrfPolicy 1 maxBound)
          overflowPolicy = required "overflow policy" (Signed.mkSignedCsrfPolicy maxBound 1)
    ...
    it "keeps every public CSRF state typed, bounded, and redacted" $ do
      let currentKeyId = required "current CSRF key ID" (Signed.mkCsrfKeyId "current")
          previousKeyId = required "previous CSRF key ID" (Signed.mkCsrfKeyId "previous")
          signingKey = required "CSRF signing key" (Signed.mkCsrfSigningKey "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA")
          previousSigningKey = required "previous CSRF signing key" (Signed.mkCsrfSigningKey "MTExMTExMTExMTExMTExMTExMTExMTExMTExMTExMTE")
          cookieLifetime = required "CSRF cookie lifetime" (Csrf.mkCsrfCookieMaxAgeSeconds 42)
          policy = required "CSRF policy" (Signed.mkSignedCsrfPolicy 42 7)
          otherPolicy = required "other CSRF policy" (Signed.mkSignedCsrfPolicy 43 7)
          keyring = required "CSRF keyring" (Signed.mkSignedCsrfKeyring currentKeyId ((currentKeyId, signingKey) :| [(previousKeyId, previousSigningKey)]))
          otherKeyring = required "other CSRF keyring" (Signed.mkSignedCsrfKeyring previousKeyId ((previousKeyId, previousSigningKey) :| [(currentKeyId, signingKey)]))
          differentActiveSigningKeyring = required "CSRF keyring with a different active signing key" (Signed.mkSignedCsrfKeyring currentKeyId ((currentKeyId, previousSigningKey) :| [(previousKeyId, previousSigningKey)]))
          differentVerificationKeyring = required "CSRF keyring with a different verification set" (Signed.mkSignedCsrfKeyring currentKeyId ((currentKeyId, signingKey) :| []))
      ...
      generatedSigningKey <- Signed.generateCsrfSigningKey
...
signedProtection :: Signed.SignedCsrfKeyring -> Signed.SignedCsrfPolicy -> IO Time.UnixTimeNanoseconds -> (context -> IO Csrf.CsrfBindingResolution) -> Csrf.CsrfProtection context
signedProtection keyring policy currentTime resolveBinding =
  Signed.signedCsrfProtection
    Signed.SignedCsrfDependencies
      { Signed.signedCsrfDependenciesKeyring = keyring,
        Signed.signedCsrfDependenciesPolicy = policy,
        Signed.signedCsrfDependenciesCurrentTime = currentTime,
        Signed.signedCsrfDependenciesResolveBinding = resolveBinding
      }
