...
import HarchWeb.ClientActionFailure (HarchClientFailure (ActionResponseProtocolFailed, ResponseApplicationFailed, StorageCleanupFailed))
...
import HarchWeb.Server (clientActionFailureDestinations, noClientActionFailureDestinations)
  ...
  describe "toWaiApplication" $ do
            ...
            baseApplication
              { HarchWeb.clientActionEndpointMetadata = \methodValue pathValue _ ->
                ...
                handleClientAction = \_ -> do
                  ...
                  pure (Just (ClientActionSucceeded (ClientActionResponse Http.status204 [] Nothing StayOnCurrentRoute noClientStorageCleanup noClientActionFailureDestinations [] [] [])))
              }
    ...
    it "uses the route-resolved context for captured actions and returns typed region patches" $ do
      ...
      let failureAttribute = Observability.ObservabilityAttribute "error.type" (Observability.TextAttribute "RegistrationStoreUnavailable")
          actionApplication =
            sampleApplication
              { HarchWeb.clientActionEndpointMetadata = \methodValue pathValue _ ->
                  if methodValue == "POST" && pathValue == "/es/known"
                    then Just (HarchWeb.routeEndpointMetadata sampleApplication KnownRoute)
                    else Nothing,
                HarchWeb.clientActionRoute = \methodValue pathValue _ ->
                  if methodValue == "POST" && pathValue == "/es/known"
                    then Just KnownRoute
                    else Nothing,
                applicationClientActionFailureRoute =
                  Just $ \clientFailure _ ->
                    case clientFailure of
                      StorageCleanupFailed -> KnownRoute
                      ActionResponseProtocolFailed -> DataRoute
                      ResponseApplicationFailed -> MissingRoute,
      ...
      maybeCapturedActionRequest
        `shouldBe` Just
          ClientActionRequest
            { clientActionRouteRequest = RouteRequest KnownRoute spanishContext,
      ...
      readResponseBody response
        >>= (`shouldSatisfy` Text.isInfixOf "\"failureNavigation\":{\"storage-cleanup-failed\":\"/es/known\",\"action-response-protocol-failed\":\"/data\",\"response-application-failed\":\"/404\"}")
      maybeRequestObservability <- readIORef requestObservabilityReference
      ...
      expectAll
        ( (responseStatus responseBodyValue `shouldBe` Http.status422)
            :| [ responseContentType responseBodyValue `shouldBe` "application/json; charset=utf-8",
                 ...
                 encodedResponse `shouldSatisfy` Text.isInfixOf "\"failureNavigation\":null",
    ...
    it "maps codec unknown, method, malformed, invalid-decoder, and domain action outcomes to safe protocol responses" $ do
      ...
      let actionApplication =
            sampleApplication
              { HarchWeb.clientActionEndpointMetadata = \methodValue pathValue _ ->
                  if methodValue == "POST" && pathValue `elem` ["/known", "/invalid"]
                    then Just (HarchWeb.routeEndpointMetadata sampleApplication KnownRoute)
                    else Nothing,
                HarchWeb.clientActionRoute = \methodValue pathValue _ ->
                  if methodValue == "POST" && pathValue `elem` ["/known", "/invalid"]
    ...
    it "compares and prints every non-page response form without observing stream identity" $ do
      ...
      let responseBodyValue = ResponseBody Http.status200 "text/plain" "ok" [] [] []
          ...
          otherActionResponse = ClientActionResponse Http.status422 [] (Just (literalElementId "email")) StayOnCurrentRoute noClientStorageCleanup noClientActionFailureDestinations [] [] []
    ...
    it "serializes action responses with no patches or focus target" $ do
      let actionApplication =
            sampleApplication
              { HarchWeb.clientActionEndpointMetadata = \methodValue pathValue _ ->
                  if methodValue == "POST" && pathValue == "/actions/empty"
                    then Just (HarchWeb.routeEndpointMetadata sampleApplication KnownRoute)
                    else Nothing,
                HarchWeb.clientActionRoute = \methodValue pathValue _ ->
                  if methodValue == "POST" && pathValue == "/actions/empty"
