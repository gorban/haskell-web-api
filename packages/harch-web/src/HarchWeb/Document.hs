...
defaultNavigationRuntimeScript =
  Text.unlines
    [ "(() => {",
      ...
      "      return;",
      "    }",
      "    captureKernel.drain().forEach((capturedEvent) => dispatchCapturedEvent(captureKernel.eventTypes, capturedEvent));",
      "  }",
      "",
      "  function dispatchCapturedEvent(capturedEventTypes, capturedEvent) {",
      "    switch (capturedEvent.type) {",
      "      case capturedEventTypes.Submit:",
