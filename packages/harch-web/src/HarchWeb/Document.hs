...
defaultNavigationRuntimeScript =
  Text.unlines
    [ "(() => {",
      ...
      "    if (!actionResponse || !Array.isArray(actionResponse.patches)) {",
      "      throw new Error('invalid action response patches');",
      "    }",
      "    actionResponse.patches.forEach((patch) => {",
      "      if (!patch || typeof patch.id !== 'string' || typeof patch.html !== 'string') {",
      "        throw new Error('invalid action response patch');",
      "      }",
      "      const currentRegion = document.getElementById(patch.id);",
      "      if (!currentRegion) { throw new Error('missing action response region'); }",
      ...
      "      if (!replacementRegion || replacementRegion.nextElementSibling) { throw new Error('invalid action response replacement'); }",
      "      currentRegion.replaceWith(replacementRegion);",
      "    });",
      "    if (!actionResponse.navigation && actionResponse.focusId !== null && actionResponse.focusId !== undefined) {",
      "      if (typeof actionResponse.focusId !== 'string') { throw new Error('invalid action response focus'); }",
      "      document.getElementById(actionResponse.focusId)?.focus();",
      "    }",
      "    return actionResponse.navigation || null;",
      "  }",
      "",
      "  function validFailureReference(value) {",
      "    return typeof value === 'string' && /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/.test(value);",
      "  }",
      "",
      "  function failureReference(actionResponse, response) {",
      "    const actionReference = actionResponse && actionResponse.requestId;",
      "    if (validFailureReference(actionReference)) { return actionReference; }",
      "    const responseReference = response && response.headers && response.headers.get('X-Request-ID');",
      "    return validFailureReference(responseReference) ? responseReference : null;",
      "  }",
      "",
      "  function failureDestination(actionResponse, failure) {",
      "    const candidate = actionResponse && actionResponse.failureNavigation && actionResponse.failureNavigation[failure];",
      "    if (typeof candidate !== 'string') { return null; }",
      "    try {",
      "      const destination = new URL(candidate, window.location.href);",
      "      return destination.origin === window.location.origin ? destination.href : null;",
      "    } catch (_error) {",
      "      return null;",
      "    }",
      "  }",
      "",
      "  function replaceWithClientActionFailure(reference) {",
      "    activeNavigation?.abortController.abort();",
      "    activeNavigation = null;",
      "    document.documentElement.replaceChildren();",
      "    const head = document.createElement('head');",
      "    const title = document.createElement('title');",
      "    title.textContent = 'Request could not be completed';",
      "    head.append(title);",
      "    const body = document.createElement('body');",
      "    const main = document.createElement('main');",
      "    const heading = document.createElement('h1');",
      "    heading.textContent = 'Request could not be completed';",
      "    const message = document.createElement('p');",
      "    message.textContent = 'The page was cleared to protect your account. Please contact support and provide this request ID.';",
      "    main.append(heading, message);",
      "    if (reference) {",
      "      const referenceText = document.createElement('p');",
      "      referenceText.textContent = reference;",
      "      main.append(referenceText);",
      "    }",
      "    body.append(main);",
      "    document.documentElement.append(head, body);",
      "  }",
      "",
      "  function replaceWithTerminalActionDocument(documentText) {",
      "    activeNavigation?.abortController.abort();",
      "    activeNavigation = null;",
      "    document.open();",
      ...
      "  }",
      "",
      "  function presentClientActionFailure(failure, actionResponse, response) {",
      "    const destination = failureDestination(actionResponse, failure);",
      "    if (destination) {",
      "      window.location.assign(destination);",
      "      return;",
      "    }",
      "    replaceWithClientActionFailure(failureReference(actionResponse, response));",
      "  }",
      "",
      "  function cleanClientStorage(actionResponse) {",
      "    if (!actionResponse || !Array.isArray(actionResponse.storageCleanup)) {",
      "      throw new Error('invalid client storage cleanup declaration');",
      "    }",
      "    const seen = new Set();",
      "    const failures = [];",
      "    actionResponse.storageCleanup.forEach((entry) => {",
      "      if (!entry || (entry.storage !== 'local' && entry.storage !== 'session') || typeof entry.key !== 'string' || entry.key.length === 0 || entry.key.length > 256) {",
      "        throw new Error('invalid client storage cleanup entry');",
      "      }",
      "      const identity = entry.storage + ':' + entry.key;",
      "      if (seen.has(identity)) { throw new Error('duplicate client storage cleanup entry'); }",
      "      seen.add(identity);",
      "      try {",
      "        (entry.storage === 'local' ? window.localStorage : window.sessionStorage).removeItem(entry.key);",
      "      } catch (_error) {",
      "        failures.push(new Error('client storage cleanup failed'));",
      "      }",
      "    });",
      "    if (failures.length > 0) { throw new AggregateError(failures, 'client storage cleanup failed'); }",
      "  }",
      "",
      ...
      "    let response;",
      "    try {",
      "      response = await window.fetch(actionUrl, {",
      ...
      "      });",
      "    } catch (_error) {",
      "      return { failure: 'action-response-protocol-failed', response: null, actionResponse: null };",
      "    }",
      "    const contentType = response.headers.get('Content-Type') || '';",
      "    if (response.status === 500 && contentType.toLowerCase().startsWith('text/html')) {",
      "      return { terminalDocument: await response.text(), response };",
      "    }",
      "    let actionResponse;",
      "    try { actionResponse = await response.json(); } catch (_error) {",
      "      return { failure: 'action-response-protocol-failed', response, actionResponse: null };",
      "    }",
      "    if (response.status === 401 && response.headers.get('X-Harch-Action-Authentication') === 'navigate') {",
      "      return { authenticationNavigationRequired: true, actionResponse, response };",
      "    }",
      "    if (response.status === 401 && response.headers.get('X-Harch-Action-Reauthenticate') === 'required') {",
      "      return { reauthenticationRequired: true };",
      "    }",
      "    return { actionResponse, response, reauthenticationRequired: false, responseSucceeded: response.ok };",
      "  }",
      "",
      ...
      "        if (outcome.failure) {",
      "          presentClientActionFailure(outcome.failure, outcome.actionResponse, outcome.response);",
      "          return;",
      "        }",
      "        if (outcome.terminalDocument) {",
      "          if (!settlement.completed()) { return; }",
      "          replaceWithTerminalActionDocument(outcome.terminalDocument);",
      "          return;",
      "        }",
      ...
      "          return;",
      "        }",
      "        if (outcome.authenticationNavigationRequired) {",
      "          if (!settlement.recoverable()) { return; }",
      "          let navigation;",
      "          try { navigation = applyActionResponse(outcome.actionResponse); } catch (_error) {",
      "            presentClientActionFailure('response-application-failed', outcome.actionResponse, outcome.response);",
      "            return;",
      "          }",
      "          if (navigation) { await navigateActionResponse(navigation); }",
      "          return;",
      "        }",
      "        if (!outcome.responseSucceeded) {",
      "          if (!settlement.recoverable()) { return; }",
      "          try { applyActionResponse(outcome.actionResponse); } catch (_error) {",
      "            presentClientActionFailure('response-application-failed', outcome.actionResponse, outcome.response);",
      "          }",
      "          return;",
      "        }",
      ...
      "          return;",
      "        }",
      "        try { cleanClientStorage(outcome.actionResponse); } catch (_error) {",
      "          presentClientActionFailure('storage-cleanup-failed', outcome.actionResponse, outcome.response);",
      "          return;",
      "        }",
      "        let navigation;",
      "        try { navigation = applyActionResponse(outcome.actionResponse); } catch (_error) {",
      ...
      "        }",
      "      } catch (_error) {",
      "        presentClientActionFailure('response-application-failed', null, null);",
      "      }",
      "    });",
      "  }",
      "",
...
-- succeeds. A non-success action response first settles recoverably, then may
-- present its typed patch; a stale response therefore cannot affect a later
-- page selected by navigation. A
...
defaultCaptureKernelScript =
  Text.unlines
    [ "(() => {",
      ...
      "  const isActionable = (state) => state === actionState.Pending || state === actionState.Claimed || state === actionState.Recoverable || state === actionState.Retained;",
      ...
      "    if (cancel) {",
      "      cancel.hidden = !isActionable(state);",
      "    }",
      "  };",
      ...
      "    const needsWarning = [...capturedActions.values()].some((entry) => requiresLeaveConfirmation(entry) && isActionable(entry.state));",
