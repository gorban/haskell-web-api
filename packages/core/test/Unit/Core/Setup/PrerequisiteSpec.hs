      ...
      expectAll
        ( ( Prerequisite.parseTracingEndpoint "http://collector:4318/v1/traces"
            ...
            :| [ Prerequisite.parseTracingEndpoint "https://collector.example/v1/traces"
                   `shouldBe` Right Prerequisite.TcpEndpoint {Prerequisite.tcpEndpointHost = "collector.example", Prerequisite.tcpEndpointPort = 443},
                 Prerequisite.parseTracingEndpoint "http://[::1]:4318/v1/traces"
                   `shouldBe` Right Prerequisite.TcpEndpoint {Prerequisite.tcpEndpointHost = "::1", Prerequisite.tcpEndpointPort = 4318},
                 Prerequisite.parseTracingEndpoint "https://[::1]/v1/traces"
                   `shouldBe` Right Prerequisite.TcpEndpoint {Prerequisite.tcpEndpointHost = "::1", Prerequisite.tcpEndpointPort = 443},
                 Prerequisite.parseTracingEndpoint "https://collector/v1/traces"
                   `shouldBe` Right Prerequisite.TcpEndpoint {Prerequisite.tcpEndpointHost = "collector", Prerequisite.tcpEndpointPort = 443}
               ]
        )
    ...
    it "rejects malformed or unsupported tracing endpoints explicitly" $ do
      expectAll
        ( (Prerequisite.parseTracingEndpoint "://collector:4318/v1/traces" `shouldBe` Left (Prerequisite.InvalidTracingEndpointFormat "://collector:4318/v1/traces"))
            :| [ Prerequisite.parseTracingEndpoint "collector:4318/v1/traces" `shouldBe` Left (Prerequisite.InvalidTracingEndpointFormat "collector:4318/v1/traces"),
                 Prerequisite.parseTracingEndpoint "grpc://collector:4317" `shouldBe` Left (Prerequisite.UnsupportedTracingEndpointScheme "grpc"),
                 Prerequisite.parseTracingEndpoint "http:///v1/traces" `shouldBe` Left Prerequisite.MissingTracingEndpointHost,
                 Prerequisite.parseTracingEndpoint "http://:4318/v1/traces" `shouldBe` Left Prerequisite.MissingTracingEndpointHost,
                 Prerequisite.parseTracingEndpoint "http://collector:not-a-port/v1/traces" `shouldBe` Left (Prerequisite.InvalidTracingEndpointPort "not-a-port"),
                 Prerequisite.parseTracingEndpoint "http://collector:0/v1/traces" `shouldBe` Left (Prerequisite.InvalidTracingEndpointPort "0"),
                 Prerequisite.parseTracingEndpoint "http://[::1/v1/traces" `shouldBe` Left Prerequisite.MissingTracingEndpointHost,
                 Prerequisite.parseTracingEndpoint "http://[]:4318/v1/traces" `shouldBe` Left Prerequisite.MissingTracingEndpointHost,
                 Prerequisite.parseTracingEndpoint "http://[::1]suffix/v1/traces" `shouldBe` Left (Prerequisite.InvalidTracingEndpointFormat "suffix")
               ]
        )
      ...
      expectAll
        ( (parseError `shouldBe` Prerequisite.InvalidTracingEndpointPort "not-a-port")
            :| [ parseError `shouldNotBe` Prerequisite.InvalidTracingEndpointPort "other-port",
                 ...
                 show [parseError] `shouldBe` "[InvalidTracingEndpointPort \"not-a-port\"]",
                 show (Prerequisite.InvalidTracingEndpointFormat "bad-endpoint") `shouldBe` "InvalidTracingEndpointFormat \"bad-endpoint\"",
                 show (Prerequisite.UnsupportedTracingEndpointScheme "grpc") `shouldBe` "UnsupportedTracingEndpointScheme \"grpc\"",
                 show Prerequisite.MissingTracingEndpointHost `shouldBe` "MissingTracingEndpointHost"
               ]
        )
