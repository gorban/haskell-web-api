...
openapi_compatibility_step="$(step_body 'Verify released OpenAPI compatibility stack')"
grep -Fxq "        if: steps.tls_compatibility.outcome == 'success'" <<<"$openapi_compatibility_step" || fail "OpenAPI compatibility verification must wait for the TLS compatibility stack"
grep -Fxq '          tools/test-openapi3-compatibility-stack.sh' <<<"$openapi_compatibility_step" || fail "OpenAPI compatibility step must run the isolated released-source suites"
