...
The first slice is intentionally incomplete. `web-api` has not yet replaced its two-language
helpers with an application message enum/catalog, the compile-time message-template quasiquoter is
not yet present, and the required third-locale example has not yet landed. EI remains open until
those migrations and proof points are complete; the native ICU dependency is not a claim that the
full localization authoring experience already exists.
